<?php
/**
 * Template Name: kiddushreserve Template
 */

if (!defined('DONOTCACHEPAGE')) define('DONOTCACHEPAGE', true);

if (!isset($_POST['originalJd'])) {
	// shouldn't be here. Redirect back to kiddushmain.php
	header('Location: /kiddushmain');
	die;
}
require_once(get_template_directory() . '/includes/kiddush.php');
get_header();
?>
<script LANGUAGE="JavaScript" type="text/javascript">
function verify() {
	var themessage = "You are required to complete the following fields: ";
	if (document.form.realname.value=="") {
		themessage = themessage + " - Your full name";
	}
	if (document.form.email.value=="") {
		themessage = themessage + " -  Your E-mail";
	}
	//alert if fields are empty and cancel form submit
	if (themessage == "You are required to complete the following fields: ") {
		document.form.submit();
	}
	else {
		alert(themessage);
		return false;
	}
}
</script>
<div align="left">
<form name="form" action="/kiddushreserve" method="post" action="">
	<input type=hidden name="originalJd" value="<?= $_POST['originalJd'] ?>">
	<input type=hidden name="form" value="submitted">
	<table align="left" border="0" width="82%" id="table1" background="pic/chabad-bg.gif" height="118">
		<tr>
			<td>
<?php if (isset($_POST['form']) && $_POST['form'] == "submitted"): ?>
	<p align="center" class="font-weight-bold"><?= reserveDate($_POST) ?><br></p>
<?php
//if they need to fill out the form to sign up for a Kiddush
elseif(!isset($_POST['change'])): ?>
			</td>
		</tr>
		<tr>
			<td align="center" colspan="2">
            
            <p><strong>Simply fill out this form</strong></p>
				
			
			  </td>
		</tr>
		<tr>
		  <td colspan="2" align="center">
          <p class="text-danger">*Required Fields.</p>
          
           </td>
	  </tr>
		<tr>
			<td colspan="2">
				<p align="center" class="font-weight-bold">You are reserving the week of 
				<?= datetime_from_jd($_POST['originalJd'])->format("M j, Y") ?></p>
			</td>
		</tr>
		<tr>
			<td><span class="chabad"><font color="#FF0000">*</font> Your
					Full Name</span>
			</td>
			<td><span class="chabad">
					<input TYPE="TEXT" SIZE="24" MAXSIZE="50" NAME="realname"></span>
			</td>
		</tr>
		<tr>
			<td><font color="#FF0000">*</font> Your Email</td>
			<td><span class="chabad">
					<input TYPE="TEXT" SIZE="24" MAXSIZE="50" NAME="email"></span>
			</td>
		</tr>
		<tr>
			<td>
				<p class="chabad"><font color="#FF0000">*</font> Re-type Email</p></td>
			<td>
				<input TYPE="TEXT" SIZE="24" MAXSIZE="50" NAME="elll0">
			</td>
		</tr>
		<tr>
			<td class="chabad"><font color="#FF0000">*</font> Phone</td>
			<td class="chabad">
				<input TYPE="TEXT" SIZE="24" MAXSIZE="50" NAME="phone"></td>
		</tr>
		<tr>
			<td class="chabad">In Honor Of or Note</td>
			<td class="chabad">&nbsp;</td>
		</tr>
		<tr>
			<td class="chabad">&nbsp;</td>
			<td class="chabad"><textarea rows="3" name="honor" cols="30"></textarea></td>
		</tr>
		<tr>
			<td colspan="2">
				<p align="center"><span class="chabad"></td>
		</tr>
		<tr>
			<td colspan="2">
				<p align="center">
				<input type="button" value="Reserve Kiddush" onclick="verify();"> 
<?php else: // if the "Change" button was clicked
	$reservation = getReservation($_POST['originalJd']);
	if ($reservation): ?>
			</td>
		</tr>
		<tr>
			<td colspan="2" align="center" bgcolor="#FFFFFF">
			<p align="left" class="chabad-header">Simply fill out this form&nbsp;
			<font color="#FF0000"><br>
			*Required Fields </font> </td>
		</tr>
		<tr>
			<td><span class="chabad"><font color="#FF0000">*</font> Your
						Email</span> 
			</td>
			<td><span class="chabad">
					<input TYPE="TEXT" SIZE="24" MAXSIZE="50" NAME="email"></span>
			</td>
		</tr>
		<tr>
			<td width="46%" class="chabad"><font color="#FF0000">*</font> New Date</td>
			<td width="54%" class="chabad">
				<select NAME="newJd">
					<?php foreach(getAllowedChangeDays($_POST['originalJd']) as $day): ?>
					<option value="<?= $day ?>"><?= datetime_from_jd($day)->format("M j, Y") ?></option>
					<?php endforeach ?>
				</select>
			</td>
		</tr>
		<tr>
			<td colspan="2">
				<p align="center">
				<input type="submit" name="change" value="Reserve Kiddush"> 
	<?php else: ?>
				<font color='red'>Could not find reservation</font>
	<?php endif ?>
<?php endif ?>
			</td>
		</tr>
	</table>
</form>
</div>
<?php
get_footer();
